...
class RegisterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnRegister.setOnClickListener {
            ...
            if (gender == genders[0]) {
                showError(tvError, "Please select your gender.")
                return@setOnClickListener
            }
            ...
            if (password.length < 6) {
                showError(tvError, "Password should be at least 6 characters.")
                return@setOnClickListener
            }
            if (pin.length != 4) {
                showError(tvError, "Transaction PIN must be exactly 4 digits.")
                return@setOnClickListener
            }
            if (!cbTerms.isChecked) {
                showError(tvError, "Please accept the Terms and Privacy Policy.")
                return@setOnClickListener
            }
            ...
            if (!success) {
                showError(tvError, "An account with this email already exists.")
                return@setOnClickListener
            }
